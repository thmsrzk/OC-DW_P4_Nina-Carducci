...
element.wrap(`<div class='item-column mb-4${columnClasses}'></div>`)}else{console.error(`Columns should be defined as numbers or objects. ${typeof columns} is not supported.`)}},moveItemInRowWrapper(element){element.appendTo(".gallery-items-row")},responsiveImageItem(element){if(element.prop("tagName")==="IMG"){element.addClass("img-fluid")}},openLightBox(element,lightboxId){let src=element.attr("src").replace(".webp","_fullsize.webp");$(`#${lightboxId}`).find(".lightboxImage").attr("src",src);$(`#${lightboxId}`).modal("toggle")},prevImage(){let activeImage=null;$("img.gallery-item").each(function(){if($(this).attr("src").replace("_fullsize","")===$(".lightboxImage").attr("src").replace("_fullsize","")){activeImage=$(this)}});let activeTag=$(".tags-bar span.active-tag").data("images-toggle");let imagesCollection=[];if(activeTag==="all"){$(".item-column").each(function(){if($(this).children("img").length){imagesCollection.push($(this).children("img"))}})}else{$(".item-column").each(function(){if($(this).children("img").data("gallery-tag")===activeTag){imagesCollection.push($(this).children("img"))}})}
let index=0,prev=null;$(imagesCollection).each(function(i){if($(activeImage).attr("src").replace("_fullsize","")===$(this).attr("src").replace("_fullsize","")){index=i-1}});prev=imagesCollection[index]||imagesCollection[imagesCollection.length-1];let src=$(prev).attr("src").replace(".webp","_fullsize.webp");$(".lightboxImage").attr("src",src)},nextImage(){let activeImage=null;$("img.gallery-item").each(function(){if($(this).attr("src").replace("_fullsize","")===$(".lightboxImage").attr("src").replace("_fullsize","")){activeImage=$(this)}});let activeTag=$(".tags-bar span.active-tag").data("images-toggle");let imagesCollection=[];if(activeTag==="all"){$(".item-column").each(function(){if($(this).children("img").length){imagesCollection.push($(this).children("img"))}})}else{$(".item-column").each(function(){if($(this).children("img").data("gallery-tag")===activeTag){imagesCollection.push($(this).children("img"))}})}
let index=0,next=null;$(imagesCollection).each(function(i){if($(activeImage).attr("src").replace("_fullsize","")===$(this).attr("src").replace("_fullsize","")){index=i+1}});next=imagesCollection[index]||imagesCollection[0];let src=$(next).attr("src").replace(".webp","_fullsize.webp");$(".lightboxImage").attr("src",src)},createLightBox(gallery,lightboxId,navigation){gallery.append(`<aside class="modal fade" id="${
